...
def equations(state, policy_state):
    ...
    if (Version == 'cjl') or (Version == '2007'):
        ...
        # ----------------------------------------------------------------------- #
        loss_dict['foc_kplus'] = tf.math.exp(gr_tfp + gr_lab) * lambd_haty \
            - beta_hat * E_t(
                lambda s, ps:
                PolicyState.lambd_haty(ps) * (
                    Tstep * Definitions.Omega(s, ps)
        ...
        # ----------------------------------------------------------------------- #
        budget = Tstep* Omega * zetax * kx**alpha - Tstep * con \
            + (1 - delta)**Tstep * kx - tf.math.exp(gr_tfp + gr_lab) * kplusy
        ...
        loss_dict['foc_TATplus'] = etaAT_haty - beta_hat * E_t(
            lambda s, ps:
            PolicyState.lambd_haty(ps)
            * (Tstep  * Definitions.Omega_prime(s, ps)) * State.zetax(s)
        ...
        # ----------------------------------------------------------------------- #
        budget = Tstep*(1 - Omega) * zetax * kx**alpha - Tstep*con \
            + (1 - delta)**Tstep * kx - tf.math.exp(gr_tfp + gr_lab) * kplusy
        loss_dict['foc_lambd'] = budget
        # ----------------------------------------------------------------------- #
