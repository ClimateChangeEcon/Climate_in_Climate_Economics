...
def set_conf(cfg):
    ...
    # OPTIMIZER
    setattr(sys.modules[__name__], "optimizer", getattr(tf.keras.optimizers,cfg.optimizer)(learning_rate=cfg.learning_rate, clipvalue=cfg.clipvalue))
    ...
    def initialize_states():    
        ...
        for i,s in enumerate(cfg.states):
            if 'init' in s:
                init_val = tf.tensor_scatter_nd_update(init_val, [[j,i] for j in range(init_val.shape[0])], getattr(rng,s["init"]["distribution"])(shape=(N_sim_batch,), **s["init"]["kwargs"]))
        return init_val
