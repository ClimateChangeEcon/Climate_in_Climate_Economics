...
def gr_tfp(state, policy_state):
    ...
    if (Version == '2016') or (Version == '2007'):
        _adj_coef = Parameters.adj_coef
        _gr_tfp = (gA0hat * _adj_coef) * tf.math.exp(- Tstep *deltaA * _t)
    return _gr_tfp
...
def scc(state, policy_state):
    ...
    if (Version == '2007'):
        _dvdk =  (_lambd_haty * (Tstep * (1 - _Theta) * _Omega * alpha * _kx**(alpha-1)
                               + (1 - delta)**Tstep)
                    + (-_nuAT_haty) * (1 - _muy) * Tstep * _sigma * _zetax * _tfp * _lab * alpha
                    * _kx**(alpha-1))
    elif (Version == '2016'):
        _dvdk =  (_lambd_haty * (Tstep * (1 - _Theta - _Omega) * alpha * _kx**(alpha-1)
                               + (1 - delta)**Tstep)
...
def MATplus(state, policy_state):
    ...
    _MATplus = (1-_phi12) * _MATx + _phi21 * _MUOx \
        + Tstep*(1 - _muy) * _sigma * _tfp * _lab * _zetax * _kx**alpha + Tstep*_Eland
    return _MATplus
...
#------------Alternative SCC--------------------------------------------------#
def dMATplus_dk(state, policy_state):
    _tfp = tfp(state, policy_state)
    _lab = lab(state, policy_state)
    _muy = PolicyState.muy(policy_state)
    _zetax = State.zetax(state)
    ...
    _dMATplus_dk =  (1 - _muy) * _sigma \
                * _tfp * _lab \
                * alpha * _kx**(alpha - 1)

    return _dMATplus_dk

def dMATplus_dMAT(state, policy_state):
    _phi12 = phi12(state, policy_state)
    return 1-_phi12

def dMATplus_dMUO(state, policy_state):
    _phi21 = phi21(state, policy_state)
    return _phi21

def dMUOplus_dMAT(state, policy_state):
    _phi12 = phi12(state, policy_state)
    return _phi12

def dMUOplus_dMUO(state, policy_state):
    _phi21 = phi21(state, policy_state)
    _phi23 = phi23(state, policy_state)
    return 1 - _phi21 - _phi23

def dMUOplus_dMLO(state, policy_state):
    _phi32 = phi32(state, policy_state)
    return _phi32

def dMLOplus_dMUO(state, policy_state):
    _phi23 = phi23(state, policy_state)
    return _phi23

def dMLOplus_dMLO(state, policy_state):
    _phi32 = phi32(state, policy_state)
    return 1 - _phi32

def dTATplus_dMAT(state, policy_state):
    _MATx = State.MATx(state)
    _varphi1 = varphi1(state, policy_state)

    _dTATplus_dMAT =  _varphi1 * f2xco2 * (1 / (
        tf.math.log(2.) * _MATx))

    return _dTATplus_dMAT

def dTATplus_dTAT(state, policy_state):
    _varphi21 = varphi21(state, policy_state)
    _xi2 = xi2(state, policy_state)
    return 1 - _varphi21 - _xi2

def dTATplus_dTOC(state, policy_state):
    _varphi21 = varphi21(state, policy_state)
    return _varphi21

def dTOCplus_dTAT(state, policy_state):
    _varphi4 = varphi4(state, policy_state)
    return _varphi4

def dTOCplus_dTOC(state, policy_state):
    _varphi4 = varphi4(state, policy_state)
    return 1 - _varphi4

def du_dk(state, policy_state):
    _tfp = tfp(state, policy_state)
    _lab = lab(state, policy_state)
    _gr_tfp = gr_tfp(state, policy_state)
    _gr_lab = gr_lab(state, policy_state)
    ...
    _du_dk = (_con)**(-1/psi) * (
            (1 - _Omega - _Theta) * alpha * _kx**(alpha-1) + (1 - delta)
            )
    return _du_dk

def du_dtat(state, policy_state):
    _tfp = tfp(state, policy_state)
    _lab = lab(state, policy_state)
    _gr_tfp = gr_tfp(state, policy_state)
    _gr_lab = gr_lab(state, policy_state)
    _con = con(state, policy_state)
    _kx = State.kx(state)
    _Omega_prime = Omega_prime(state, policy_state)

    _du_dtat =  (_con)**(-1/psi) * (
            (- _Omega_prime) * _kx**alpha
            )
    return _du_dtat

def coef_5(state, policy_state):
    g56 = dTATplus_dTOC(state, policy_state)
    g66 = dTOCplus_dTOC(state, policy_state)
    _beta = beta_hat(state, policy_state)
    _coef_5 = _beta * g56 / (1 - _beta * g66)
    return _coef_5

def dvdtat(state, policy_state):
    _du_dtat = du_dtat(state, policy_state)
    _beta = beta_hat(state, policy_state)
    g55 = dTATplus_dTAT(state, policy_state)
    g65 = dTOCplus_dTAT(state, policy_state)
    _coef_5 = coef_5(state, policy_state)
    _dvdtat = _du_dtat/(1 - _beta * g55 - _beta * g65 * _coef_5)
    return _dvdtat

def coef_3(state, policy_state):
    g34 = dMUOplus_dMLO(state, policy_state)
    g44 = dMLOplus_dMLO(state, policy_state)
    _beta = beta_hat(state, policy_state)
    _coef_3 = _beta * g34 / (1 - _beta * g44)
    return _coef_3

def coef_2(state, policy_state):
    g23 = dMATplus_dMUO(state, policy_state)
    g33 = dMUOplus_dMUO(state, policy_state)
    g43 = dMLOplus_dMUO(state, policy_state)
    _coef_3 = coef_3(state, policy_state)
    _beta = beta_hat(state, policy_state)
    _coef_2 = _beta * g23 / (1 - _beta * g33 - _beta *_coef_3 * g43)
    return _coef_2

def dvdmat(state, policy_state):
    g52 = dTATplus_dMAT(state, policy_state)
    g22 = dMATplus_dMAT(state, policy_state)
    g32 = dMUOplus_dMAT(state, policy_state)
    _dvdtat = dvdtat(state, policy_state)
    _coef_2 = coef_2(state, policy_state)
    _beta = beta_hat(state, policy_state)
    _dvdmat = _beta * g52 * _dvdtat / (1. - _beta * g22 - _beta * _coef_2 * g32)
    return _dvdmat

def dvdk(state, policy_state):
    g21 = dMATplus_dk(state, policy_state)
    _du_dk = du_dk(state, policy_state)
    _beta = beta_hat(state, policy_state)
    _dvdmat = dvdmat(state, policy_state)
    _dvdk = _du_dk + _beta * _dvdmat * g21
    return _dvdk

def alter_scc(state, policy_state):
    _dvdmat = dvdmat(state, policy_state)
    _dvdk = dvdk(state, policy_state)
    _tfp = tfp(state, policy_state)
    _lab = lab(state, policy_state)
    _scc = - _dvdmat/_dvdk *_tfp * _lab
    return _scc
